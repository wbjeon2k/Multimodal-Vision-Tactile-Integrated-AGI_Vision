...
def ssim(img1, img2, mask, window_size=7, size_average=True):
    channel = img1.size(-3)
...
###############3
def gaussian_v2(window_size, sigma):
    gauss = torch.Tensor([exp(-(x - window_size // 2) ** 2 / float(2 * sigma ** 2)) for x in range(window_size)])
    return gauss / gauss.sum()

def create_window_v2(window_size, channel):
    _1D_window = gaussian_v2(window_size, 1.5).unsqueeze(1)
    _2D_window = _1D_window.mm(_1D_window.t()).float().unsqueeze(0).unsqueeze(0)
    window = Variable(_2D_window.expand(channel, 1, window_size, window_size).contiguous())
    return window

def ssim_v2(img1, img2, window_size=7, size_average=True):
    channel = img1.size(-3)
    window = create_window_v2(window_size, channel)

    if img1.is_cuda:
        window = window.cuda(img1.get_device())
    window = window.type_as(img1)

    return _ssim_v2(img1, img2, window, window_size, channel, size_average)

def _ssim_v2(img1, img2, window, window_size, channel, size_average=True):
    mu1 = F.conv2d(img1, window, padding=window_size // 2, groups=channel)
    mu2 = F.conv2d(img2, window, padding=window_size // 2, groups=channel)
