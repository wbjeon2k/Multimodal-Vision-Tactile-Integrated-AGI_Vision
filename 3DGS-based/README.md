...
```
conda create -n mip-splatting-cu121 python=3.10 -y
conda activate mip-splatting-cu121
...
# 4) Tell PyTorch which GPU arch to compile for
#   A100 -> 8.0 ; V100 -> 7.0 ; RTX 30xx -> 8.6 ; 4090 -> 8.9
#   (If unsure, run `nvidia-smi` and tell me the GPU model.)
export TORCH_CUDA_ARCH_LIST="8.0"     # <-- A100 example
# export TORCH_CUDA_ARCH_LIST="7.0"   # <-- V100 example
# export TORCH_CUDA_ARCH_LIST="8.6"   # <-- 3080/3090 example
# export TORCH_CUDA_ARCH_LIST="8.9"   # <-- 4090 example
...
pip install -v --use-pep517 --no-build-isolation .
```
...
```
python train.py -s "/nfs4/jjlee/datasets/emertics/images_relative_lit/(Msh)Sofa_Red" -m results/Sofa_Red --eval
```
